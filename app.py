...
DEFAULT_CONFIG = {
    "site_title": "XENON HOSTING  VPS",
    "site_header": "XENON HOSTING VPS",
            ...
            with open(CONFIG_FILE, 'r') as f:
                ...
                if 'passwords' in config:
                    if 'secret' in config['passwords'] and len(config['passwords']['secret']) > 20:
                        config['passwords']['secret'] = "ANISUR_RAHAMAN"
                    if 'user' in config['passwords'] and len(config['passwords']['user']) > 20:
                        config['passwords']['user'] = "XENON_ANIS"
                return config
        except:
...
def send_welcome(message):
    welcome_text = (
        "🤖 *XENON HOSTING VPS Telegram Bot*\\n\\n"
...
def send_info(message):
    info_text = (
        ...
        f"• Host: XENON HOSTING VPS (24/7 Uptime)"
    )
...
def send_api_response(message):
    ...
    try:
        r = requests.get(url, timeout=15, headers={{'User-Agent': 'XENON HOSTING-TeleBot'}})
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("XENON HOSTING VPS - Starting...")
    print("=" * 50)
    print(f"Port: {port}")
    print(f"Debug: {debug}")
    print("=" * 50)
    print("Admin Password: ANISUR_RAHAMAN")
    print("User Password: XENON_ANIS")
    print("=" * 50)
